...
def input_data(): 
    ...
    if var == 1:
        ...
        with open('data_sacond_variant.csv', 'a', encoding='utf-8' ) as f:
...
def edit_data():
    ...
    with open('data_first_variant.csv', 'r', encoding='utf-8') as f_in, open(temp_file, 'w', encoding='utf-8') as f_out:
        for line in f_in:
            if name not in line:
                f_out.write(line)
            else:
    ...
    os.remove('data_first_variant.csv')  # Удаляем оригинальный файл
    os.rename(temp_file, 'data_first_variant.csv')  # Переименовываем временный файл